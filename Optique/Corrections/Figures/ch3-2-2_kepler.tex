...
\begin{tikzpicture}[scale=0.25]
    ...
    \path[name path=Atpath]
        (It) --++ ($1.3*(B1F1p)-1.3*(It)$) coordinate (Ite);
    \path[name path=Abpath]
        (Ib) --++ ($1.3*(B1F1p)-1.3*(Ib)$) coordinate (Ibe);
    ...
    \def \mut{1.8}
    \draw[orange!70, simple, name path=OBp] (O2) --++ ($\mut*(B1)-\mut*(O2)$);
    % Rayon 2
    \def \mut{3}
    ...
    \draw[Red!70, simple]
        (It2) --++
        ($(B1)-(O2)$) coordinate (Itf);
    \draw[Red!70, double]
        (Ib2) --++
        ($(B1)-(O2)$) coordinate (Ibf);
    % Actual pinceau
