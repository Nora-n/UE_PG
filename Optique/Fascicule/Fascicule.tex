...
Les rayons émergents forment un faisceau \Ul{convergent}, donnant lieu à une
\Ul{image réelle}.

\subsubsection{Sur le foyer objet}
Même principe également. Cette fois on a alors des rayons émergents parallèles
entre eux donnant une \Ul{image à l'infini}.
...
forment un faisceau \Ul{divergent} donnant lieu à une \Ul{image virtuelle}.
        ...
        \item Définition du grossissement d'un système optique. On définit
            ...
            \begin{itemize}
                \item $\t'$ l'angle sous lequel est vue l'\underline{image}
                    \underline{\underline{sans accomoder}}, donc une image
                    formée à l'infini.
            \end{itemize}
            \begin{center}
                \begin{tikzpicture}[scale=1] 
                ...
                \draw[shift={(O1)},ultra thick,<->,>=latex]
            ...
            \end{tikzpicture}
            \end{center}
            D'une manière générale, on veut que l'œil voie sans accomoder, que
            ce soit l'objet où l'image : on s'intéresse donc à l'angle de vue
            pour un œil emmétrope à $d_m$ de l'objet, et à l'angle de vue d'une
            image \underline{formée à l'infini}. Comme on va le voir, dans le
            cas présent l'image n'est pas formée à l'infini, mais ça ne change
            pas qu'on peut définir le grossissement de la loupe de manière
            absolue, en dehors du cas présent. Avec $h$ la taille de l'objet,
...
\begin{NCdefi}{Données}
    \begin{minipage}{0.4\linewidth}
        \begin{enumerate}
            ...
            \item Au \textit{proximum}, \smallbreak
                $A$ à \SI{25}{cm} \smallbreak
                ($\OA = \SI{-25}{cm}$).
        \end{enumerate}
    \end{minipage}
        ...
        \begin{center}
            \begin{tikzpicture}[scale=.8]
                ...
                \draw[brandeisblue, double] (Br) --++ (\posi,0)
                    node[above, midway, brandeisblue] {Remotum} coordinate (I);
                % Objet
                \def \pos{-2.5}
                \def \siz{1}
                \coordinate (A) at ($(O)+(\pos,0)$);
                ...
                % Grandeurs
                \node[below=15, inner sep=0] (Og) at (O) {};
                \node[below=15, inner sep=0] (Arg) at (Ar) {};
                \draw[<->, ForestGreen, dashed] (Arg) --
                node[below, midway, sloped]{\scriptsize $\SI{-\infty}{}$} (Og);
                % Grandeurs
                \node[above] (Og) at (O) {};
